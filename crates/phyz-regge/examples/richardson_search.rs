...
//!   RICH_NS=2,3,4 cargo run --example richardson_search -p phyz-regge --release
...
use phyz_regge::mesh;
use phyz_regge::richardson::richardson_extrapolation;
...
fn main() {
    ...
    let results = richardson_extrapolation(
        &ns,
        spacing,
        |n| match bg.as_str() {
            "ds" => mesh::de_sitter(n, spacing, cosmo_l),
            "kerr" => mesh::kerr(n, spacing, mass, spin, r_min),
            _ => mesh::reissner_nordstrom(n, spacing, mass, charge, r_min),
        },
        build_known,
        &params,
        seed,

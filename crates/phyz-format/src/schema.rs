...
pub fn load_phyz_model(path: &str) -> Result<PhyzSpec> {
    let json = std::fs::read_to_string(path)?;
    let spec: PhyzSpec = serde_json::from_str(&json)?;
...
pub fn save_phyz_model(path: &str, spec: &PhyzSpec) -> Result<()> {
...
pub fn export_phyz(spec: &PhyzSpec) -> Result<String> {
...
mod tests {
    ...
    #[test]
    fn test_phyz_spec_serialization() {
        ...
        let parsed: PhyzSpec = serde_json::from_str(&json).unwrap();

//! Integration tests for the tau physics engine.
...
use tau::{
    ...
    tau_rigid::{aba, crba, rnea, total_energy},
};
...
fn ball_drop_with_contacts() {
    ...
    use tau::tau_contact::find_ground_contacts;
    ...
    let (xforms, _) = tau::tau_rigid::forward_kinematics(&model, &state);
    ...
    let forces = tau::tau_contact::contact_forces(&contacts, &state, &materials, None);
    ...
    let qdd = tau::tau_rigid::aba_with_external_forces(&model, &state, Some(&spatial_forces));
...
fn gradient_consistency() {
    ...
    let fd = tau::tau_diff::finite_diff_jacobians(&model, &state, 1e-7);
    let an = tau::tau_diff::analytical_step_jacobians(&model, &state);
...
#[test]
fn aba_equals_minv_tau_minus_c() {

...
use tau_math::{DVec, Mat6, SpatialMat, SpatialVec, Vec3, Vec6};
use tau_model::{JointType, Model, State};

/// Compute joint velocity contribution S * qd for any joint type.
fn joint_velocity(joint: &tau_model::Joint, qd: &[f64]) -> SpatialVec {
...
pub fn aba_with_external_forces(
    ...
    for i in (0..nb).rev() {
        ...
        if ndof == 1 {
            ...
            let mut tau_vec = nalgebra::DVector::<f64>::zeros(ndof);
            for k in 0..ndof {
                tau_vec[k] = state.ctrl[v_idx + k] - joint.damping * state.v[v_idx + k];
            }
            ...
            let u_vec = &tau_vec - &(s_mat.transpose() * &pa_vec); // ndof x 1
    ...
    for i in 0..nb {
        ...
        if ndof == 1 {
            ...
            let pa_vec = nalgebra::DVector::from_column_slice(p_a[i].data.as_slice());
            let u_vec = &tau_vec - &(s_mat.transpose() * &pa_vec);

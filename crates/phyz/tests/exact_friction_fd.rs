...
use phyz::phyz_contact::{
    ContactMaterial, ContactProblem, ContactSolverConfig, assemble, contact_solve_differential,
    contact_solve_differential_transpose, find_contacts, find_ground_contacts_model,
    solve_contacts_warm,
};
...
fn direction(dim: usize, scale: f64) -> (Vec<f64>, Vec<f64>) {
    ...
    let dc = (0..dim)
        .map(|i| scale * 0.08 * ((i * 13) as f64 * 0.29).cos())
        .collect();
    (d_apr, dc)
}
...
fn check(deg: f64) -> Report {
    ...
    assert!(
        plain.converged,
        "{deg} deg: the base solve must converge (residual {:e})",
        plain.residual
    );
    let sliding_corners = plain
        ...
        .zip(&p.rows)
        .filter(|(f, r)| {
            f.x > 0.0 && ((f.y * f.y + f.z * f.z).sqrt() - r.mu * f.x).abs() <= 1e-9 * f.x
        })
        .count();
    ...
    assert_eq!(
        ...
        "{deg} deg: differentiation changed the primal"
    );
    let df = flat(&df);
    ...
    let fd_at = |h: f64| {
        ...
        assert!(
            up.converged && dn.converged,
            "{deg} deg: a perturbed solve failed to converge"
        );
        flat(&up.impulses)
            .iter()
    ...
    let s =
        FixedPointSensitivity::at(&p, &plain, &c).expect("the IFT map exists at a converged solve");
...
fn the_sliding_box_gradient_matches_a_finite_difference() {
    ...
    assert!(
        r.sliding_corners >= 4,
        "35 deg must slide on every corner, got {}",
        r.sliding_corners
    );
    assert!(
        r.fd_vs_forward < 1e-6,
        "forward mode vs fd: {:e}",
        r.fd_vs_forward
    );
    assert!(
        r.transpose_identity < 1e-10,
        "transpose identity: {:e}",
        r.transpose_identity
    );
...
fn the_slow_slide_gradient_matches_a_finite_difference() {
    ...
    assert!(
        r.sliding_corners >= 4,
        "27.5 deg must slide on every corner, got {}",
        r.sliding_corners
    );
    assert!(
        r.fd_vs_forward < 1e-6,

...
use crate::math::{DMat, DVec, SpatialMat, SpatialTransform, SpatialVec, Vec3};
use crate::model::{JointType, Model, State};

/// Compute joint velocity contribution S * qd for any joint type.
fn joint_velocity(joint: &crate::model::Joint, qd: &[f64]) -> SpatialVec {
...
pub fn aba_with_external_forces(
    ...
    let qfrc = crate::rigid::actuation::generalized_forces(model, state);
    let armature = crate::rigid::actuation::armature_diag(model);
...
mod tests {
    use super::*;
    use crate::math::{GRAVITY, Mat3, SpatialInertia};
    use crate::model::ModelBuilder;

    fn make_double_pendulum() -> crate::model::Model {
        let length = 1.0;
        let mass = 1.0;
        ModelBuilder::new()

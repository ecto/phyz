...
//!
//! A [`ContactMaterial`] is attached to a [`crate::Body`] via
//! [`crate::Body::material`], and [`crate::Model::contact_materials`] turns
//! those into the per-body slice the contact solver assembles against. Bodies
//! that carry `None` take the scene material, so a model that sets nothing is
//! unchanged.
//!
//! Read [`ContactMaterial::combine`] before choosing which body gets a
//! material: friction combines by `max`, so a grippy body grips everything it
//! touches, not only the surface you had in mind.
...
impl ContactMaterial {
    ...
    /// `phyz_contact::convex::regularization_diag`), so a single contact resolving a
    ...
    /// `phyz_contact`'s `ContactRow::from_material` clamps the stabilization bias at
    ...
    ///
    ///   **This is the rule that decides which body you attach a material
    ///   to**, and it is asymmetric in a way that bites. A material lives on
    ///   a [`crate::Body`] and applies to *every* contact that body is part
    ///   of, so `max` means a grippy body grips everything it touches.
    ///   Measured on a skateboarding robot: grip tape (`mu = 1.5`) put on the
    ///   *deck* correctly grips the shoes — but the deck's bare underside then
    ///   also grips the asphalt during the tail scrape, which braked the board
    ///   from 0.6 to 0.39 m/s and killed the trick. The same 1.5 on the
    ///   *shoes* raises only the foot-on-deck contacts and leaves
    ///   deck-on-ground at wood's 0.6. Put the material on the part whose
    ///   surface it describes, then check what else that part touches.

...
use tau_collision::{AABB, Collision, gjk_distance_rot, sweep_and_prune};
use tau_math::{SpatialVec, Vec3};
use tau_model::{Geometry as ModelGeometry, Model, State};

/// Convert tau_model::Geometry to tau_collision::Geometry.
fn convert_geometry(g: &ModelGeometry) -> tau_collision::Geometry {
    match g {
        ModelGeometry::Sphere { radius } => tau_collision::Geometry::Sphere { radius: *radius },
        ModelGeometry::Capsule { radius, length } => tau_collision::Geometry::Capsule {
            radius: *radius,
            length: *length,
        },
        ModelGeometry::Box { half_extents } => tau_collision::Geometry::Box {
            half_extents: *half_extents,
        },
        ModelGeometry::Cylinder { radius, height } => tau_collision::Geometry::Cylinder {
            radius: *radius,
            height: *height,
        },
        ModelGeometry::Mesh { vertices, faces } => tau_collision::Geometry::Mesh {
            vertices: vertices.clone(),
            faces: faces.clone(),
        },
        ModelGeometry::Plane { normal } => tau_collision::Geometry::Plane { normal: *normal },
    }
}

...
use phyz_gpu::contact_pipeline::GroundContactParams;
...
fn contact(friction: f64) -> GroundContactParams {
    GroundContactParams {
        ground_height: 0.0,
...
fn flat_heightfield_matches_flat_plane() {
    ...
    sim_plane.enable_ground_contact(0.0, TIME_CONST, 1.0, 0.8).unwrap();
    sim_hf
        .enable_contact_terrain(contact(0.8), None, &[], Some(&Heightfield::flat(0.0)))
        .unwrap();

    let st = model.default_state();
    sim_plane.load_states(&[st.clone()]);
...
fn uniform_slope_matches_tilted_gravity() {
    ...
    sim_hf
        .enable_contact_terrain(contact(0.0), None, &[], Some(&flat))
        .unwrap();
    ...
    sim_tilt.enable_ground_contact(0.0, TIME_CONST, 1.0, 0.0).unwrap();
...
fn ball_settles_at_the_bottom_of_a_bowl() {
    ...
    sim.enable_contact_terrain(contact(0.4), None, &[], Some(&hf))
        .unwrap();
...
fn gpu_matches_cpu_on_bumpy_field() {
    ...
    gpu.enable_contact_terrain(contact(0.8), None, &[], Some(&hf))
        .unwrap();
    let st = model.default_state();
    gpu.load_states(&[st.clone()]);

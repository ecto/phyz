...
/// Contact response time constant (MuJoCo's solref default) and the
/// natural frequency it names: `omega = 1/tc`. Mass-proportional gains
/// `k = m*omega^2` give every body the same frequency, so the resting
/// penetration is `g/omega^2` — independent of mass.
...
fn gpu_contact_stops_a_dropped_box() {
    ...
    // weight. With mass-proportional stiffness k = m*omega^2, that depth is
